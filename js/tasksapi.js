/*
     _            _                                                __         _                  
    | |          | |                                              / _|       | |                 
    | |_ __ _ ___| | _____   _ __ _   _ _ __   ___ _ __ ___  _ __| |_ _ __ __| |  _ __ ___   ___ 
    | __/ _` / __| |/ / __| | '__| | | | '_ \ / __| '_ ` _ \| '__|  _| '__/ _` | | '_ ` _ \ / _ \
    | || (_| \__ \   <\__ \_| |  | |_| | | | | (__| | | | | | |  | | | | | (_| |_| | | | | |  __/
     \__\__,_|___/_|\_\___(_)_|   \__, |_| |_|\___|_| |_| |_|_|  |_| |_|  \__,_(_)_| |_| |_|\___|
                                   __/ |                                                         
                                  |___/                                                          
       
                         (generated by http://patorjk.com/software/taag)
*/

// ------------------------------------------ Initializing ------------------------------------------

var tasks = {
     isInitialised: false,
     init: function(callback){
       if(!callback){console.error('Callback function has not been provided');return}
       gapi.load('client:auth2', function() {
         gapi.client.init({
           apiKey: 'AIzaSyArBQrznPzgD5aU_NKPWkorEaklGkIBouM',
           clientId: '248150601049-fbibbrvjeqojdj45csgilhmj2vk7240e.apps.googleusercontent.com',
           discoveryDocs: ["https://www.googleapis.com/discovery/v1/apis/tasks/v1/rest"],
           scope: 'https://www.googleapis.com/auth/tasks'
         }).then(function() {
           tasks.isInitialised = true;
           tasks.callback(callback)
         })
       });
     },
     auth: {
       isLoggedIn: function(){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         return gapi.auth2.getAuthInstance().isSignedIn.get();
       },
       login: function(callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         gapi.auth2.getAuthInstance().signIn().then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       },
       logout: function(callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         gapi.auth2.getAuthInstance().signOut().then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       }
     },
     taskLists: {
       get: function(callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasklists.list().then(function(response){
           if(callback){
             tasks.callback(callback, response.result.items)
           }
         });
       },
       add: function(title, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasklists.insert({
           title: title
         }).then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       },
       remove: function(taskListId, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasklists.delete({
           tasklist: taskListId
         }).then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       },
       edit: function(taskListId, title, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasklists.update({
           tasklist: taskListId,
           id: taskListId,
           title: title
         }).then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       }
     },
     tasks: {
       get: function(taskListId, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}     
         gapi.client.tasks.tasks.list({
           tasklist: taskListId
         }).then(function(response){
           if(callback){
             tasks.callback(callback, response.result.items)
           }
         });
       },
       add: function(taskListId, params, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasks.insert({
           tasklist: taskListId,
           resource: params
         }).then(function(response){
           if(callback){
             tasks.callback(callback, response)
           }
         });
       },
       remove: function(taskListId, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}       
         gapi.client.tasks.tasks.delete({
           tasklist: taskListId,
           task: taskId,
           id: taskId
         }).then(function(){
           if(callback){
             tasks.callback(callback)
           }
         });
       },
       edit: function(taskListId, taskId, params, callback){
         if(!tasks.isInitialised){console.error('Function "tasks.init()" must be called first'); return}
         else if(arguments.length==0){console.error('Incorrect function parameters');return;}    
         gapi.client.tasks.tasks.patch({
           tasklist: taskListId,
           task: taskId,
           resource: params
         }).then(function(response){
           if(callback){
             tasks.callback(callback, response)
           }
         });
       },
       complete: function(taskListId, taskId, callback){
         gapi.client.tasks.tasks.patch({
           tasklist: taskListId,
           task: taskId,
           resource: {'status':'completed'}
         }).then(function(response){
           if(callback){
             tasks.callback(callback, response)
           }
         });
       }
     },
     callback: function(callback, params){
       if(arguments.length==0){console.error('Incorrect function parameters');return;}
       if(!params){callback()}
       else{callback(params);}
     }
   }
 